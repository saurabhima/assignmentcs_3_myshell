...
int main(int argc, char **argv) {
	...
	while( token_pipe != NULL ) 
   	{
      	...
      	count++;
      		if(token_pipe!=NULL)
      		{
       		strcpy(parameter[count-1],token_pipe);
   	...
   	while( token != NULL ) 
   {
    ...
    int i;
    argv[0] = malloc(strlen("mypipe") * sizeof(char));
    argv[1] = malloc(strlen(command) * sizeof(char));
    strcpy(argv[0],"mypipe");
